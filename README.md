...
- Telegram Customer Service Agent: planned system agent for Telegram Bot customer service. The Agent Hub tools, permissions, event trigger, API token template, and human-takeover semantics are registered; the Telegram Bot API/Webhook connector is still the remaining execution layer.
...
- [ ] Telegram Bot connector execution layer: implement Bot API/Webhook ingestion, conversation persistence, operator console, AI auto-replies, customer/lead linking, and human takeover controls.
...
- Telegram Customer Service Agent：规划中的 Telegram Bot 智能客服系统 Agent。Agent Hub 工具、权限、事件触发、API Token 模板和人工接管语义已注册；Telegram Bot API/Webhook 连接器仍是后续执行层。
...
- [ ] Telegram Bot 连接器执行层：实现 Bot API/Webhook 入站、会话持久化、座席界面、AI 自动回复、客户/Lead 关联和人工接管控制。
