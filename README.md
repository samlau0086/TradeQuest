...
- [-] 将 Inbox 重构为类似 HubSpot 的统一沟通工作区。第一轮可见改造已经在推进中：Inbox 已具备更接近 CRM 工作台的页面头部、更轻的工作区壳层、更清晰的会话队列控制、更新后的会话卡片和更明确的空状态，同时保留现有 Email / WhatsApp / Live Chat / Telegram 行为。最新一轮还把 Live Chat / Telegram 头部控制、访客证据卡片、Email 跟踪/评论区域，以及左侧队列工具条 / 批量操作工作区继续拉齐到同一套更轻的 CRM 视觉语言。
- [ ] 保留统一左侧会话列表，并支持渠道筛选、保存视图、批量操作和分配控制。
- [-] Email、WhatsApp、Live Chat、Telegram 使用一致的中间会话/详情区域。Email、WhatsApp、Live Chat 和 Telegram 已开始使用试点布局：消息/正文和内部备注保留在主区域，渠道上下文和 Agent 建议在大屏进入独立右侧 rail；详情头部的动作按钮、状态 badge，以及 owner/stage 工具条也开始收敛到共享 conversation toolbar primitives。
...
- [-] 保留各渠道特有能力，如邮件 WYSIWYG、WhatsApp 媒体、翻译、定时发送和人工接管，同时统一布局模式。内嵌 WhatsApp 工作区已开始迁移到同一套 CRM 壳层语言；Live Chat 和 Telegram 的控制区也已开始采用一致的轻量 badge / button 表达，同时保留接管、运行 Agent、翻译和联系人关联等动作。
