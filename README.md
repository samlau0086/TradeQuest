...
- Telegram Customer Service Agent: planned system agent for Telegram Bot customer service. Inbound webhook ingestion, persistence, notifications, and Agent Hub events are registered; outbound Bot API sending, desk UI, AI auto-replies, and human takeover controls remain roadmap items.
...
- Telegram Customer Service Agent：规划中的 Telegram Bot 智能客服系统 Agent。入站 webhook、消息持久化、通知和 Agent Hub 事件已接入；Bot API 出站发送、座席界面、AI 自动回复和人工接管控制仍是后续 roadmap。
