...
- [x] 已将客户 / Lead 详情中的智能区块切到共享 CRM 作战室卡片语言。`ClientWorkroomOverview`、`ClientWorkroomOperationsStrip`、`ClientWorkroomIntelligence`、`ClientWorkroomHistory` 和 `ClientAiRadarCard` 现在都复用同一套轻量 section-card / toolbar primitives，并统一了推荐下一步、情报、RAG 依据和待处理队列的中英双语表达。
- [x] 已继续将客户详情右侧 sidebar rail 收敛到同一套浅色 CRM 作战室系统。`ClientDetailsSidebarColumn`、`ClientProfileSidebarWidgets`、`ClientFollowUpAgentWidget`、`ClientQuotesWidget`、`ClientContactsWidget` 和 `ClientConversationNotesWidget` 已脱离旧的深色工具栏语言，开始和中间主作战区共用同一套记录页节奏。
- [x] 已将客户详情顶部 identity zone 继续改造成更像 HubSpot 的记录页头部。`ClientDetailsHeader` 与 `ClientIdentityPanel` 现在会以更清晰的 CRM header / property summary 结构展示记录身份、状态、来源、评分、联系人快照和产品关注点，而不是旧的 modal 式标题条。
- [x] 已将记录页活动区继续收口为更像 HubSpot 的 activity feed。`ClientEventPanel` 现在已用共享的浅色 CRM 作战室语言承载时间线、事件列表和成长日志 tab，并统一了中英双语标题、空状态、展开按钮和活动卡片层次。
- [x] Move the record-detail intelligence widgets onto the shared CRM workroom card language. `ClientWorkroomOverview`, `ClientWorkroomOperationsStrip`, `ClientWorkroomIntelligence`, `ClientWorkroomHistory`, and `ClientAiRadarCard` now use the same light section-card / toolbar primitives, with cleaner bilingual labels for next-step, intelligence, RAG evidence, and work queue surfaces.
- [x] Restyle the record sidebar rail toward the same light CRM workroom system. `ClientDetailsSidebarColumn`, `ClientProfileSidebarWidgets`, `ClientFollowUpAgentWidget`, `ClientQuotesWidget`, `ClientContactsWidget`, and `ClientConversationNotesWidget` now use the lighter record-page rail language instead of the older dark utility rail, so the customer detail page reads more like one coordinated workspace.
- [x] Refactor the top record identity zone toward a HubSpot-like record header. `ClientDetailsHeader` and `ClientIdentityPanel` now present record identity, status, source, score, contact snapshot, and product focus in a clearer CRM header/property summary structure instead of the older modal-style title strip.
- [x] Restyle the record activity workspace toward a HubSpot-like activity feed. `ClientEventPanel` now uses the shared light CRM workroom language for the timeline, event list, and growth log tabs, with cleaner bilingual labels, lighter activity cards, and unified expand/empty states.

### Phase 3: Unified Communication Workspace

- [-] Refactor Inbox into a HubSpot-like communication workspace. The first visible pass is now in progress: the Inbox uses a CRM-style page header, lighter workspace shell, cleaner conversation queue controls, refreshed conversation cards, and a more intentional empty workspace state while keeping the existing email/WhatsApp/Live Chat/Telegram behavior intact. The latest pass also brings Live Chat / Telegram header controls, visitor evidence cards, Email tracking/comments surfaces, the left-side queue toolbar / bulk action workspace, quick-view presets, and the conversation queue cards themselves into the same lighter CRM workspace language. The sidebar is now being regrouped into clearer CRM sections such as workspace controls and conversation queue, with the queue header, queue cards, and left-side control copy cleaned up into a more readable CRM operator sidebar instead of the older mixed/garbled pass. A shared `ConversationWorkspaceShell` now also wraps Email, WhatsApp, Live Chat, and Telegram detail panes so the next round of HubSpot-style conversation UI changes can happen from one shared scaffold instead of four separate channel layouts. The shared conversation header, summary strip, and context rail are now also being normalized onto one CRM workroom baseline, reducing the remaining channel-by-channel chrome drift. The latest pass now also aligns the message timeline, reply composer, internal notes panel, and follow-up strip onto the same CRM card language, so the middle-pane interaction layer is starting to feel like one product instead of four channel-specific tools. A new shared `ConversationSectionCard` / `ConversationSectionHeader` layer now also backs Email detail sub-panels and the WhatsApp composer workspace, and has started absorbing the WhatsApp meta strip and agent-context support surfaces too. The standalone WhatsApp modal itself is now mounted through the same split workroom assembly path as the embedded Inbox view, and the shared split-pane / context-rail primitives have been upgraded into a cleaner light CRM shell so the next visual pass can focus more on product behavior than channel-specific markup cleanup. The newest queue sweep also rewrites the page shell, left operator sidebar, queue results header, list cards, and bulk-action panel onto a cleaner shared CRM queue language, so the Inbox now reads much more like an operator workbench than a stack of ad hoc channel lists. The Inbox top bar itself is now isolated as `InboxWorkspaceHeader`, rewritten away from the older garbled `PageHeader` copy into a more explicit workspace summary with KPI cards and action buttons, while `InboxWorkspaceEmptyState` has been rebuilt as a lighter “pick a conversation” work surface that reads more like a CRM shell than a placeholder. The latest conversation-shell pass also restyles `ConversationWorkspaceShell`, `ConversationSplitPane`, `ConversationDetailHeader`, `ConversationRecordSummaryStrip`, and `ConversationContextRail` into a more explicit workroom hierarchy, so Email / WhatsApp / Live Chat / Telegram now share a cleaner header-summary-evidence structure before the next timeline/composer sweep. The newest middle-pane cleanup also rewrites `ConversationMessageList` and `ConversationReplyComposer` away from the old garbled Chinese pass, giving Telegram / Live Chat a cleaner CRM timeline, clearer inbound/outbound labeling, and a more explicit reply desk with shared bilingual copy. The latest shared-header sweep also upgrades `ConversationDetailHeader` and `ConversationRecordSummaryStrip` into a more explicit CRM operator shell, while Email detail cards now read more like a record workspace with clearer subject/reading guidance, a framed rendered-message area, richer tracking summaries, and a more intentional internal-note composer. The newest multi-channel pass also brings the shared `ConversationInternalNotesPanel` back onto the same CRM note-composer language as Email, and further cleans the WhatsApp timeline / reply composer copy so WhatsApp, Live Chat, and Telegram middle panes keep converging toward one communication workroom instead of three adjacent chat tools. The latest right-rail pass now also brings Live Chat customer intelligence, visitor evidence cards, Telegram / Live Chat agent entry labels, and the WhatsApp agent-context intro card onto a more consistent card rhythm and bilingual CRM copy, so the evidence-and-suggestions rail is starting to behave like one shared context system instead of four unrelated side panels. Under the hood, the queue sorting / owner-filter orchestration, saved-view persistence layer, content-panel prop assembly, dialog-layer prop assembly, sidebar prop assembly, and page-shell assembly have also started moving out of `Inbox.tsx` into dedicated hooks, making the main Inbox page behave more like a workspace assembler instead of a monolithic state container. The latest cleanup also introduces `useInboxWorkspaceData` as a shared Inbox workspace snapshot hook, `useInboxPageShellInputs` as a dedicated page-shell input builder, `useInboxConversationSlices` as a dedicated selected-record/conversation state packer, `useInboxQueueWorkspace` as a dedicated left-rail selection/bulk-action coordinator, `InboxConversationSidebarTypes` as a shared queue-rail contract, `InboxQueueSidebarWorkspace` as the dedicated left-rail workspace surface, and a cleaned-up multilingual email reply-extraction utility, so more of the Inbox page now behaves like an orchestrated workspace shell instead of a single giant controller component.
- [-] Refactor Inbox into a HubSpot-like communication workspace. The first visible pass is now in progress: the Inbox uses a CRM-style page header, lighter workspace shell, cleaner conversation queue controls, refreshed conversation cards, and a more intentional empty workspace state while keeping the existing email/WhatsApp/Live Chat/Telegram behavior intact. The latest pass also brings Live Chat / Telegram header controls, visitor evidence cards, Email tracking/comments surfaces, the left-side queue toolbar / bulk action workspace, quick-view presets, and the conversation queue cards themselves into the same lighter CRM workspace language. The sidebar is now being regrouped into clearer CRM sections such as workspace controls and conversation queue, with the queue header, queue cards, and left-side control copy cleaned up into a more readable CRM operator sidebar instead of the older mixed/garbled pass. A shared `ConversationWorkspaceShell` now also wraps Email, WhatsApp, Live Chat, and Telegram detail panes so the next round of HubSpot-style conversation UI changes can happen from one shared scaffold instead of four separate channel layouts. The shared conversation header, summary strip, and context rail are now also being normalized onto one CRM workroom baseline, reducing the remaining channel-by-channel chrome drift. The latest pass now also aligns the message timeline, reply composer, internal notes panel, and follow-up strip onto the same CRM card language, so the middle-pane interaction layer is starting to feel like one product instead of four channel-specific tools. A new shared `ConversationSectionCard` / `ConversationSectionHeader` layer now also backs Email detail sub-panels and the WhatsApp composer workspace, and has started absorbing the WhatsApp meta strip and agent-context support surfaces too. The standalone WhatsApp modal itself is now mounted through the same split workroom assembly path as the embedded Inbox view, and the shared split-pane / context-rail primitives have been upgraded into a cleaner light CRM shell so the next visual pass can focus more on product behavior than channel-specific markup cleanup. The newest queue sweep also rewrites the page shell, left operator sidebar, queue results header, list cards, and bulk-action panel onto a cleaner shared CRM queue language, so the Inbox now reads much more like an operator workbench than a stack of ad hoc channel lists. The Inbox top bar itself is now isolated as `InboxWorkspaceHeader`, rewritten away from the older garbled `PageHeader` copy into a more explicit workspace summary with KPI cards and action buttons, while `InboxWorkspaceEmptyState` has been rebuilt as a lighter “pick a conversation” work surface that reads more like a CRM shell than a placeholder. The latest conversation-shell pass also restyles `ConversationWorkspaceShell`, `ConversationSplitPane`, `ConversationDetailHeader`, `ConversationRecordSummaryStrip`, and `ConversationContextRail` into a more explicit workroom hierarchy, so Email / WhatsApp / Live Chat / Telegram now share a cleaner header-summary-evidence structure before the next timeline/composer sweep. Under the hood, the queue sorting / owner-filter orchestration, saved-view persistence layer, content-panel prop assembly, dialog-layer prop assembly, sidebar prop assembly, and page-shell assembly have also started moving out of `Inbox.tsx` into dedicated hooks, making the main Inbox page behave more like a workspace assembler instead of a monolithic state container. The latest cleanup also introduces `useInboxWorkspaceData` as a shared Inbox workspace snapshot hook, `useInboxPageShellInputs` as a dedicated page-shell input builder, `useInboxConversationSlices` as a dedicated selected-record/conversation state packer, `useInboxQueueWorkspace` as a dedicated left-rail selection/bulk-action coordinator, `InboxConversationSidebarTypes` as a shared queue-rail contract, `InboxQueueSidebarWorkspace` as the dedicated left-rail workspace surface, and a cleaned-up multilingual email reply-extraction utility, so more of the Inbox page now behaves like an orchestrated workspace shell instead of a single giant controller component. The latest visible operator-rail cleanup also removes another batch of garbled Chinese queue copy, promotes the left KPI strip into clearer Visible / Follow-up / Unread / Unassigned cards, and tightens the queue header / list-item owner-stage copy so the sidebar is starting to read more like a polished CRM operator queue instead of a patched migration layer.
- [-] Refactor Inbox into a HubSpot-like communication workspace. The first visible pass is now in progress: the Inbox uses a CRM-style page header, lighter workspace shell, cleaner conversation queue controls, refreshed conversation cards, and a more intentional empty workspace state while keeping the existing email/WhatsApp/Live Chat/Telegram behavior intact. The latest pass also brings Live Chat / Telegram header controls, visitor evidence cards, Email tracking/comments surfaces, the left-side queue toolbar / bulk action workspace, quick-view presets, and the conversation queue cards themselves into the same lighter CRM workspace language. The sidebar is now being regrouped into clearer CRM sections such as workspace controls and conversation queue, with the queue header, queue cards, and left-side control copy cleaned up into a more readable CRM operator sidebar instead of the older mixed/garbled pass. A shared `ConversationWorkspaceShell` now also wraps Email, WhatsApp, Live Chat, and Telegram detail panes so the next round of HubSpot-style conversation UI changes can happen from one shared scaffold instead of four separate channel layouts. The shared conversation header, summary strip, and context rail are now also being normalized onto one CRM workroom baseline, reducing the remaining channel-by-channel chrome drift. The latest pass now also aligns the message timeline, reply composer, internal notes panel, and follow-up strip onto the same CRM card language, so the middle-pane interaction layer is starting to feel like one product instead of four channel-specific tools. A new shared `ConversationSectionCard` / `ConversationSectionHeader` layer now also backs Email detail sub-panels and the WhatsApp composer workspace, and has started absorbing the WhatsApp meta strip and agent-context support surfaces too. The standalone WhatsApp modal itself is now mounted through the same split workroom assembly path as the embedded Inbox view, and the shared split-pane / context-rail primitives have been upgraded into a cleaner light CRM shell so the next visual pass can focus more on product behavior than channel-specific markup cleanup. The newest queue sweep also rewrites the page shell, left operator sidebar, queue results header, list cards, and bulk-action panel onto a cleaner shared CRM queue language, so the Inbox now reads much more like an operator workbench than a stack of ad hoc channel lists. The Inbox top bar itself is now isolated as `InboxWorkspaceHeader`, rewritten away from the older garbled `PageHeader` copy into a more explicit workspace summary with KPI cards and action buttons, while `InboxWorkspaceEmptyState` has been rebuilt as a lighter “pick a conversation” work surface that reads more like a CRM shell than a placeholder. The latest conversation-shell pass also restyles `ConversationWorkspaceShell`, `ConversationSplitPane`, `ConversationDetailHeader`, `ConversationRecordSummaryStrip`, and `ConversationContextRail` into a more explicit workroom hierarchy, so Email / WhatsApp / Live Chat / Telegram now share a cleaner header-summary-evidence structure before the next timeline/composer sweep. Under the hood, the queue sorting / owner-filter orchestration, saved-view persistence layer, content-panel prop assembly, dialog-layer prop assembly, sidebar prop assembly, and page-shell assembly have also started moving out of `Inbox.tsx` into dedicated hooks, making the main Inbox page behave more like a workspace assembler instead of a monolithic state container. The latest cleanup also introduces `useInboxWorkspaceData` as a shared Inbox workspace snapshot hook, `useInboxPageShellInputs` as a dedicated page-shell input builder, `useInboxConversationSlices` as a dedicated selected-record/conversation state packer, `useInboxQueueWorkspace` as a dedicated left-rail selection/bulk-action coordinator, `InboxConversationSidebarTypes` as a shared queue-rail contract, `InboxQueueSidebarWorkspace` as the dedicated left-rail workspace surface, and a cleaned-up multilingual email reply-extraction utility, so more of the Inbox page now behaves like an orchestrated workspace shell instead of a single giant controller component. The latest visible operator-rail cleanup also removes another batch of garbled Chinese queue copy, promotes the left KPI strip into clearer Visible / Follow-up / Unread / Unassigned cards, and tightens the queue header / list-item owner-stage copy so the sidebar is starting to read more like a polished CRM operator queue instead of a patched migration layer. The next visible pass now also aligns `InboxWorkspaceHeader` and `InboxSidebarControls` onto the same operator-shell language: cleaner Chinese control copy, clearer workspace summary text, stronger sync-status expression, and more deliberate KPI / action framing so the outer shell reads more like one CRM workbench instead of separate header and filter widgets.
...
- [x] 已将共享 `AgentContextSuggestions` 面板重做为更轻的 CRM 作战室卡片语言。分析模式、缓存状态、当前判断、RAG 证据、冲突提醒、待跟进安排和建议动作现在都统一落在共享 section-card / toolbar primitives 上，不再沿用旧的深色工具面板壳层。
- [x] Restyle the shared `AgentContextSuggestions` panel onto the same light CRM workroom language. Analysis mode, cached state, current reading, RAG evidence, conflict warnings, follow-up scheduling, and suggested actions now sit on shared section-card / toolbar primitives instead of the older dark utility panel.
...
- [x] 已将体量过大的 `useInboxPageShellInputs` 再拆成独立的输入契约与 builder 模块（`useInboxPageShellInputTypes`、`inboxPageShellInputBuilders`），让 page-shell wiring 可以继续瘦身，而不会重新长回单体 hook。
- [x] 已开始对 `ComposeEmail.tsx` 使用同样的渐进式抽离方式：头部、收件人与元信息表单、附件画廊、底部动作条已经进入独立的 `ComposeEmailSections` 基元，方便后续把外发邮件工作区继续往 HubSpot 风格演进，而不用反复打开一个超大的 JSX 组件。
- [x] 已对 `InboxSidebarControls.tsx` 做同样的结构拆分：当前视图摘要、搜索与标签、队列设置、已保存视图四块已进入独立的 `InboxSidebarControlSections`，后续把左侧队列工作区重排成更像 HubSpot 的 operator rail 时，就不需要继续在单文件里混排所有控制区。
- [x] 已将 `useActiveConversationContext.ts` 进一步拆成独立的多渠道上下文 builders（`activeConversationContextBuilders`），把 WhatsApp 选中态、Telegram 上下文、Live Chat 证据/上下文、可关联联系方式解析、统一 conversation 定位这些职责从协调 hook 中拆开，方便后续分别演进和测试。
- [x] 已将 `useInboxConversationSlices.ts` 拆成独立的 slice 类型契约与 builder（`useInboxConversationSliceTypes`、`inboxConversationSliceBuilders`），把 selected email、active conversation、comments、follow-up、translation 这五块状态装配从单一 hook 中拆开，继续压缩 Inbox 周边的编排复杂度。
- [x] 已新增 `useInboxPageShellOptions`，把 `Inbox.tsx` 中原本超长的一次性 `useInboxPageShellInputs({...})` 参数清单按 workspace / queue / selection / compose / channel workspace / conversation state 分组装配，继续把 Inbox 页面收敛为更清晰的工作区编排层。
- [x] 已将 `useInboxPageShellInputs.ts` 进一步瘦身为真正的薄 builder wrapper：现在它直接消费 memo 好的 `UseInboxPageShellInputsOptions` 对象，不再二次解构再重建一整份超长参数表，继续压缩 page-shell 管线中的重复编排层。
- [x] 已将原本仍偏大的 `inboxPageShellInputBuilders.ts` 再拆成独立的 content panel / sidebar / dialog layer / visibility 四个 builder 模块，并保留原 barrel 入口做兼容导出，进一步降低 page-shell 装配内部的耦合度。
...
- [-] 建立共享布局组件：`CRMWorkspaceLayout`、`RecordPageLayout`、`LeftListMiddleDetailRightPanelLayout`、`WidgetRail`。`CRMWorkspaceLayout` 已有初始实现，并支持 header/body 滚动控制；产品/报价工作区和客户/线索工作区已开始使用。`RecordPageLayout` 已有初始实现，客户详情布局已委托给它。`WidgetRail` 已开始承载客户详情右侧 widgets。`LeftListMiddleDetailRightPanelLayout` 已提供可复用三栏工作区骨架，Inbox 的可拖拽工作区也已开始复用它的三栏基础样式。Inbox 已升级 `ConversationContextRail`，用于承载 Email、WhatsApp、Telegram、Live Chat 的上下文和智能体建议区域，并支持标题、描述、操作区、panel/rail 样式和折叠行为。Email、WhatsApp、Live Chat 和 Telegram 的上下文/智能体建议区域已开始使用 rail 样式，并带轻量标题与折叠控制；最新一轮也把这个共享 rail 自身拉到了更统一的 CRM 卡片基座上。
- [ ] 保持 CRM 高频工作场景的紧凑、可扫读、可重复操作，而不是做成营销页风格。

### 阶段 2：客户 / Lead 详情页

- [-] 将客户/Lead 详情重构为类似 HubSpot 的销售作战室。当前已经开始第一轮可见改造：`RecordPageLayout` 切到了更轻的 CRM workspace 壳层，详情页头部也更接近作战室 header，主区 / 侧栏 / 评论区正在脱离旧的深色 modal 语言。最新一轮也开始把客户详情拆成更明确的三段：左侧身份 / 属性区、中间作战区、右侧 widget rail，为后续继续往 HubSpot 的 record workspace 信息架构靠拢打基础。
- [-] 使用清晰三栏结构：左侧身份与属性，中间活动时间线和全渠道历史，右侧 AI、任务、报价、联系人、RAG、待审批 widget。当前这一轮已让 Client Details 拥有更清晰的中部 workroom panel、更轻的事件区，以及更卡片化的右侧 widget rail；左侧身份 / 属性区还需要后续再做一轮更紧凑的信息架构整理。
- [-] 使用清晰三栏结构：左侧身份与属性，中间活动时间线和全渠道历史，右侧 AI、任务、报价、联系人、RAG、待审批 widget。当前这一轮已让 Client Details 拥有更清晰的中部 workroom panel、更轻的事件区，以及按 Record / AI / Revenue / Relationship / Knowledge 分组的右侧 widget rail；左侧身份 / 属性区还需要后续再做一轮更紧凑的信息架构整理。
- [-] 统一页面级视觉系统，包括 header、左侧身份栏、中间 workroom、右侧 widget rail。当前这一轮正在对齐 pill 样式、卡片圆角、边框与阴影语言、按钮处理方式，以及区块标题层级，让整个 record page 更像同一套 CRM workspace，而不是几个分步重构的孤岛。
- [-] 在 UI 上明确区分客户级情报和 Lead 级情报。当前 workroom panel 已先将客户级情报和 Lead 级情报拆成两个可见卡块，放在活动时间线之前；侧栏和其他 widgets 的更深层区分仍在继续推进。
- [-] 把“下一步最佳行动”作为销售工作的主入口。当前主 workroom hero panel 已把 primary recommendation、summary 和即时动作放到了时间线之前，中间 workroom 也开始继续拆成更明确的 overview / intelligence / tasks / cross-channel history 小部件层。
- [-] 在不过度拥挤主时间线的情况下展示报价、联系人、关联产品、RAG 依据、待处理任务和渠道历史。当前报价、联系人、RAG 依据、待处理事项和全渠道历史已经更明确地分布到 workroom 与 siderail 中；关联产品还需要更清晰的独立展示方式。
- [-] 在情报和执行之间增加更清晰的运营工作带。当前这一轮已开始把待跟进、任务队列、审批和内部备注提炼成中间 workroom 的 operating layer，让详情页更像销售驾驶舱而不是静态摘要页。

### 阶段 3：统一沟通工作区

- [-] 将 Inbox 重构为类似 HubSpot 的统一沟通工作区。第一轮可见改造已经在推进中：Inbox 已具备更接近 CRM 工作台的页面头部、更轻的工作区壳层、更清晰的会话队列控制、更新后的会话卡片和更明确的空状态，同时保留现有 Email / WhatsApp / Live Chat / Telegram 行为。最新一轮还把 Live Chat / Telegram 头部控制、访客证据卡片、Email 跟踪/评论区域，以及左侧队列工具条 / 批量操作工作区继续拉齐到同一套更轻的 CRM 视觉语言；左侧会话队列卡片本身也开始按更明显的状态层级、选中感和 CRM record queue 结构重排，并加入 quick view presets、更细的队列排序、负责人筛选、comfortable / compact 密度切换，以及更紧凑的 owner / stage 属性模式。当前左侧栏也已进一步重组为“工作区控制”和“对话队列”两段，并把队列头部、列表卡片、控制文案中的历史乱码替换为可读的中文/英文基座，为下一轮更像 HubSpot 的信息架构和视觉层次打底；中间详情区的共享 header / summary / context rail 基座也开始统一，后续就可以更集中地做 HubSpot 风格的 detail workspace 微调。消息列表、回复框、内部备注和待跟进条这一层也已开始统一到一套更一致的 CRM 交互语言上，最新还补出 `ConversationSectionCard` / `ConversationSectionHeader` 作为 Email detail cards 和 WhatsApp composer 的共享 section 基座。现在独立的 WhatsApp 对话弹窗也已经切回和内嵌 Inbox 相同的 split workroom 装配路径，共享的 `ConversationSplitPane` / `ConversationContextRail` 也进一步收紧为更轻的 CRM 壳层，说明后续改造可以更少在“渠道特例布局”上消耗精力，更多集中在工作区产品体验本身。最新的队列重构还把页面壳层、左侧 operator sidebar、队列结果头部、列表卡片和批量操作面板统一到一套更干净的 CRM queue 语言上，让 Inbox 更像一个运营工作台，而不是几组临时拼接的渠道列表；而这一轮又把顶部 `InboxWorkspaceHeader` 从旧的乱码 `PageHeader` 文案彻底改写为更明确的工作台摘要 + KPI cards + 操作按钮，并把 `InboxWorkspaceEmptyState` 重做成更像 CRM 工作台的“选择会话开始处理”空状态面，进一步把最外层壳子从“能用”推进到“看起来就是工作区”。这一轮还继续收紧了共享详情壳层：`ConversationWorkspaceShell`、`ConversationSplitPane`、`ConversationDetailHeader`、`ConversationRecordSummaryStrip` 和 `ConversationContextRail` 现在开始更明确地分出头部、行动摘要、主时间线和右侧证据栏层级，让四个渠道的详情区一起更接近 HubSpot 风格的作战工作台。现在 `ConversationMessageList` 与 `ConversationReplyComposer` 也已同步清掉历史乱码中文，并补齐共享的双语时间线 / 出入站标签 / 回复工作区文案，让 Telegram / Live Chat 的中间工作区开始更像统一的 CRM 会话桌面，而不是临时聊天面板。最新这一轮还把共享 `ConversationDetailHeader` 与 `ConversationRecordSummaryStrip` 进一步收紧成更明确的 operator shell / snapshot 结构，并把 Email 正文、跟踪、附件和评论区继续往 record workspace 的 section stack 整理：主题卡、阅读建议、渲染内容框、跟踪概览卡和更明确的内部备注输入区，都开始说同一套 CRM workroom 语言。现在 Telegram / Live Chat 共用的 `ConversationInternalNotesPanel` 也同步回到与 Email 评论区更接近的卡片层级、状态 pills 和输入区结构上；同时 WhatsApp 的时间线 / 发送前翻译面板 / 回复工作区文案也继续清理，让三类即时沟通渠道的中间主内容区更接近一套统一的 communication workroom。最新一轮又继续把右侧 context rail 往统一证据系统收口：Live Chat 的客户情报卡、访客证据卡、Telegram / Live Chat 的 agent 入口文案，以及 WhatsApp 的 Agent Context 开场说明卡，都开始共用更一致的卡片节奏、标题结构和中英双语 CRM 文案。底层方面，队列排序、保存视图、content panel 参数装配、dialog layer 参数装配，以及 sidebar 参数装配都开始迁移到独立 hooks，进一步减轻 `Inbox.tsx` 的页面控制器负担；这一轮又补上了 `useInboxConversationSlices` 与 `useInboxQueueWorkspace`，分别把当前选中邮件/会话的状态切片、以及左侧队列的选择/批量操作编排都从页面组件里抽离，并进一步拆出 `InboxConversationSidebarTypes` 与 `InboxQueueSidebarWorkspace`，把左侧会话工作区本身也变成可独立演进的 HubSpot 风格 sidebar surface，继续把 `Inbox.tsx` 收敛成真正的工作区编排层。最新这轮可见清理还继续把 operator rail 顶部 KPI strip 提升为更明确的可见会话 / 待跟进 / 未读 / 未分配统计卡片，并清掉 QueueHeader、列表卡片、Owner/Stage 快编区域里的又一批历史乱码中文，让左侧会话队列开始真正具备可读、可扫视的 CRM 运营栏气质；而现在 `InboxWorkspaceHeader` 与 `InboxSidebarControls` 也开始共享更统一的 operator shell 语言，包括更清楚的中文文案、同步状态表达、KPI/动作 framing 和工作区说明，进一步把外层工作台从“功能可用”推进到“结构和语气都像一个 CRM 产品”。
- [-] 保留统一左侧会话列表，并支持渠道筛选、保存视图、批量操作和分配控制。当前队列已经支持 quick views、渠道筛选、批量操作、负责人筛选、排序模式、密度切换、保存在浏览器中的 saved views、默认视图记忆和当前视图重置语义，同时补上了更清晰的当前视图摘要、结构化 saved view 展示，以及独立的队列结果头部与次级 quick filters，让左侧更像 CRM 队列工作台。最新一轮又进一步重做了队列外壳、过滤设置区、保存视图区、搜索与标签区、同步状态区、列表卡片和 inline owner/stage controls，使左侧真正具备了 HubSpot 风格 operator queue 的工作台感；剩余工作主要是继续把它演进成更完整的团队/共享视图工作流。
- [-] Email、WhatsApp、Live Chat、Telegram 使用一致的中间会话/详情区域。Email、WhatsApp、Live Chat 和 Telegram 已开始使用试点布局：消息/正文和内部备注保留在主区域，渠道上下文和 Agent 建议在大屏进入独立右侧 rail；详情头部的动作按钮、状态 badge，以及 owner/stage 工具条也开始收敛到共享 conversation toolbar primitives。最新一轮进一步升级了 `ConversationWorkspaceShell`、`ConversationDetailHeader`、`ConversationRecordSummaryStrip`、`ConversationMessageList` 和 `ConversationReplyComposer`，让中间详情区开始形成更统一的 CRM workroom 骨架；现在内嵌 WhatsApp、Email、Live Chat、Telegram 四个详情 pane 都已经完整接回这套共享骨架，说明四个核心沟通渠道都已经端到端进入新的中间工作区语言，同时仍保留映射、跟进、RAG、翻译、人工接管、渠道回复等各自能力。待跟进 strip 也已升级为更轻的 secondary action bar，让详情顶部的工作流控制层开始更像统一 CRM 工作台。现在四种会话详情头部下方也补上了共享的 `ConversationRecordSummaryStrip`，用于在消息时间线之前先展示记录摘要层；最新一轮又把它升级为更接近 HubSpot 的顶部“下一步行动 / 记录摘要”工作区，让四个渠道都拥有明确的 primary recommendation、quick actions 和记录态势摘要。内嵌 WhatsApp 详情现在也开始通过共享的 `ConversationSplitPane` 承载消息区和建议区，进一步减少一个主要的渠道特例布局分支，同时保留 WhatsApp 自身的发送、映射、翻译和 Agent 能力。最新一轮还补上了共享的 `ConversationWorkspaceShell`，让四个渠道的 header / summary / follow-up / main content / composer 结构真正进入同一层工作台骨架；并且共享的 `ConversationDetailHeader`、`ConversationRecordSummaryStrip`、`ConversationContextRail` 已开始统一到一套更干净的 CRM 卡片层级和中文文案基座上。消息列表、回复框、内部备注、待跟进条现在也开始共用更接近 HubSpot workroom 的卡片密度和交互层次；Email 的 body / attachments / tracking / comments 子面板和 WhatsApp 的 composer workspace 也开始复用共享 section-card 结构。最新一轮又把 Email 正文、附件、跟踪和评论区进一步收敛到统一的 section header / summary / actions 语言，并补上中英文标题与说明，使 Email 主工作区更接近共享 CRM workroom。现在 Telegram / Live Chat 共用的消息列表与内部备注区也已经清掉旧乱码中文，WhatsApp 顶部 summary 文案也已拉回共享 workroom 语言；同时 Telegram / Live Chat 的 Agent 建议面板标题文案与右侧 context rail 状态 pills 也开始统一，使右侧栏本身更像可操作的 CRM 上下文工作区。最新又将 `ConversationContextRail` 本身升级为共享 badge 模型，并接入 Email / Live Chat / Telegram，让右侧栏 header 的状态信息开始真正走同一套信息结构；同时 `ConversationRecordSummaryStrip` 也新增共享 status badge 能力，并接入 Email / WhatsApp / Live Chat / Telegram，使顶部 summary 状态行开始和右侧 rail 共享同类信息表达。现在 `ConversationDetailHeader` 也补上了共享 status badge 能力，并接回四个主要会话渠道，使 header / summary / rail 三层终于都开始使用一致的状态表达方式；并且 header 里的身份信息、记录状态、Owner/Stage 控制也开始被重排成更像 CRM workroom 的三段结构。与此同时，独立 WhatsApp modal 也已经回到与 Inbox 内嵌视图相同的 split workroom 装配方式，说明接下来继续统一会话 header / summary / list 的行为层时，四个渠道和独立 WhatsApp 入口都可以共用同一套布局思路。最新这一轮还开始把中间时间线画布与右侧证据栏的层级显式拉开，让详情区逐步从“平铺消息查看器”转向更像 CRM 作战室的工作台结构；共享的消息时间线和回复工作区也同步重做为更清晰的 CRM 对话表面，让 Telegram / Live Chat 不再只是普通聊天框，而更像统一会话工作台的一部分。最新的 Email 主内容层也同步升级为更一致的记录工作区表面，让正文、附件、跟踪和评论四块开始真正说同一种 CRM workroom 语言。现在 WhatsApp 的消息时间线、标签/备注区、回复工作区和 pane summary 也已进一步统一到这套 CRM workroom 语言上，使嵌入 Inbox 的 WhatsApp 体验开始明显减少“特殊聊天弹窗”的割裂感。这一轮还继续收紧了共享顶部三层：shell 背景、split pane 层级、详情 header、行动摘要区和右侧 evidence rail 都统一到了更一致的 operator workroom 语言上，所以后续可以更专注做 timeline / body / composer 的产品体验，而不是继续修外壳不一致。现在共享 `ConversationSectionCard`、`ConversationMessageList` 和 `ConversationReplyComposer` 也已被进一步升级，Telegram / Live Chat 的主区开始更像 CRM conversation timeline + bottom action desk，而 Email 的正文/附件/评论等卡片也会同步继承更一致的 record-surface 语言。
- [-] 最新这一轮还把 Inbox 内嵌 WhatsApp 详情改成通过注入 record chrome 的方式复用同一个 `WhatsAppChatModal` 装配路径，让独立弹窗和内嵌工作区终于开始共用同一个外层工作区构建器，而不是各自维护一套外壳。
- [-] 现在还新增了共享的 `ConversationChannelWorkroom` 层，用来承载 Live Chat 与 Telegram 共通的消息时间线 + 内部备注 + 右侧上下文栏 + 底部回复区结构，这样后续继续做 HubSpot 风格的会话工作区微调时，就能同时作用到这两个渠道，而不是分别改两套 pane。
- [-] 最新这一轮还新增了更底层的 `ConversationDetailWorkroom` 骨架，让 Email 这种记录型主区也开始通过与聊天型渠道相同的核心 workroom 装配层承载，从而进一步统一四个主沟通渠道的中间详情结构。
- [x] 已清理 `EmailConversationPane` 右侧 `ConversationContextRail` 的中文文案与结构残留，使 Email 中间工作区和上下文区块进入同一套共享 workroom 语言。
...
- [-] 保留各渠道特有能力，如邮件 WYSIWYG、WhatsApp 媒体、翻译、定时发送和人工接管，同时统一布局模式。内嵌 WhatsApp 工作区已开始迁移到同一套 CRM 壳层语言；Live Chat 和 Telegram 的控制区也已开始采用一致的轻量 badge / button 表达，同时保留接管、运行 Agent、翻译和联系人关联等动作。最新一轮也把 WhatsApp header 的字段标题、映射编辑区、context suggestion 区、message list 和 composer workspace 清理到更稳定的共享基座上，减少了旧中文编码和渠道专属卡片结构的残留；standalone modal 也开始共用这套轻量 workroom 语言。
