...
- [-] Create shared UI primitives for `PageHeader`, `SectionHeader`, `ActionBar`, `Toolbar`, `FilterBar`, `StatusBadge`, `OwnerStageControl`, `EmptyState`, `ConfirmDialog`, and `DataTable`. `SectionHeader`, `StatusBadge`, `EmptyState`, `IconButton`, `ActionButton`, and `ConfirmDialog` have initial implementations.
...
- [-] 建立共享 UI 基础组件：`PageHeader`、`SectionHeader`、`ActionBar`、`Toolbar`、`FilterBar`、`StatusBadge`、`OwnerStageControl`、`EmptyState`、`ConfirmDialog`、`DataTable`。`SectionHeader`、`StatusBadge`、`EmptyState`、`IconButton`、`ActionButton` 和 `ConfirmDialog` 已有初始实现。
