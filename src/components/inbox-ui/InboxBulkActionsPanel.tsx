...
export function InboxBulkActionsPanel({
  ...
  return (
    <div className="sticky top-[188px] z-[9] border-b border-slate-100 bg-slate-50/85 px-4 py-3 backdrop-blur">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <span className="rounded-full bg-white px-2.5 py-1 text-[11px] font-semibold text-slate-600 shadow-sm ring-1 ring-slate-200">
              ...
              <div className="space-y-2">
                <div className="flex gap-2">
                  <input
                    ...
                    className="min-w-0 flex-1 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-800 outline-none transition focus:border-cyan-500 focus:bg-white"
                  />
                  <button
                    type="button"
                    onClick={onAddTag}
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    onKeyDown={event => {
                      if (event.key === 'Enter') void onAddComment();
                    }}
                    placeholder={isZh ? '输入内部备注' : 'Add internal note'}
                    className="min-w-0 flex-1 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-800 outline-none transition focus:border-blue-500 focus:bg-white"
                  />
                  <button
                    type="button"
            ...
            <div className="rounded-2xl bg-white p-3 shadow-sm ring-1 ring-slate-200">
              ...
              <div className="space-y-2">
                <div className="flex gap-2">
                  ...
                  >
                    <option value="">
                      {isZh ? '负责人：未分配' : 'Owner: Unassigned'}
                    </option>
                    {currentUser && (
                      <option value={currentUser.id}>
                        {isZh ? '分配给我' : 'Assign to me'}
                      </option>
                    )}
                  </select>
                  <button
                ...
                <div className="flex gap-2">
                  <select
                    ...
                    className="min-w-0 flex-1 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-800 outline-none transition focus:border-violet-500 focus:bg-white"
                  >
                    <option value="">
                      {isZh ? '选择阶段' : 'Select stage'}
                    </option>
          ...
          <div className="rounded-2xl bg-white p-3 shadow-sm ring-1 ring-slate-200">
            ...
            <div className="flex flex-wrap gap-2">
              <input
                ...
                className="min-w-[220px] flex-1 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-800 outline-none transition focus:border-emerald-500 focus:bg-white"
              />
              <button
                type="button"
